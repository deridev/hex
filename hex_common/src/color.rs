...
impl From<Rgb<f32>> for Color {
    fn from(rgb: Rgb<f32>) -> Self {
        let r = (rgb.0[0] * 255.0) as u32;
        let g = (rgb.0[1] * 255.0) as u32;
        let b = (rgb.0[2] * 255.0) as u32;
...
impl Color {
    ...
    pub fn to_rgb(&self) -> Rgb<f32> {
        match self {
            Color::Hex(code) => {
                let r = u8::from_str_radix(&code[1..3], 16).unwrap() as f32 / 255.0;
                let g = u8::from_str_radix(&code[3..5], 16).unwrap() as f32 / 255.0;
                let b = u8::from_str_radix(&code[5..7], 16).unwrap() as f32 / 255.0;
                Rgb::from([r, g, b])
            }
            Color::Rgb(r, g, b) => {
